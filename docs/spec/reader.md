...
1. **Arabic**: title `شُوْرَةُ الْحَمْدِ [1:0]` (surah Arabic name + `[surah:ayat]`). Actions: play (current ayat via `ayatPlayer`), PlaylistPlay (start surah from here; enabled when `loadedAyatIndex == page`), copy, A-, A+.
   - PlaylistPlay detail (MainActivity.kt:1675, :1899-1900, :1611-1657): `isPlayForwardEnabled = loadedAyatIndex == page`, so only the card whose ayat was last started by its own play button is tappable. `loadedAyatIndex` is set in `toggleAudio` (:784), stays set through a pause, and resets to `-1` on `Player.STATE_ENDED` (:326-329), on surah reload (:663) and inside `playSurahFromHere` itself (:1614). Tap: `isAutoTracking = true`, stop the single-ayat player, rebuild the whole surah playlist, `seekTo(targetMediaIndex, 0)` for this card's ayat and play — a restart of the current ayat, then every following ayat automatically. 26 dp in both states; tint `theme.primary` when enabled, `Color.LightGray.copy(alpha = 0.5f)` (`#D3D3D3` at 50%) when disabled. The top bar's big play button is a different path (`toggleSurahAudio`, :799-857): it toggles the existing surah queue and never sets `isAutoTracking`. Body: Arabic ayah text 28 sp × `arabicFontScale`, right-aligned, `#222`, ends each ayah with `۝` + Eastern-Arabic numeral. Font `muhammadi_quran` intended but **commented out** → system font currently.
